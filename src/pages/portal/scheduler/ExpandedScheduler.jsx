...
import {itemsRead, schedulerItemsRead} from "@portal/scheduler/SchedulerInnerServices.jsx";
    ...
    useEffect(() => {
        const loadSchedulerItems = async () => {
            if (!isNullOrEmpty(selectedDate) && schedulerData){
                ...
                let schedulerType = 'expanded';
                if (equalString(schedulerData?.SchedulerEntityType, 2)) {
                    schedulerType ='instructor';
                }

                let items = await schedulerItemsRead(schedulerType, schedulerData, selectedDate, courts);
