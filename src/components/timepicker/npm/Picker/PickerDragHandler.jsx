...
import {
  getTimePickerInitialPointerStyle, getTimePickerRotateStyle,
  getTimePickerStandardAbsolutePosition,
  timePickerDegree2Radian, timePickerMousePosition
} from "@/components/timepicker/npm/utils/drag.jsx";
...
class PickerDragHandler extends React.PureComponent {
  ...
  getRadian(x, y) {
    ...
    sRad += timePickerDegree2Radian(this.props.rotateState.pointerRotate);
    return sRad;
  }
  ...
  handleTimePointerChange(options = {}) {
    ...
    absolutePosition = getTimePickerStandardAbsolutePosition(
        absolutePosition,
        minLength,
        maxLength
    );
  ...
  handleMouseDown(e) {
    if (!this.state.draging) {
      ...
      const pos = timePickerMousePosition(event);
      this.startX = pos.x;
      this.startY = pos.y;
  ...
  handleMouseMove(e) {
    if (this.state.draging) {
      ...
      const pos = timePickerMousePosition(e);
      ...
      if (this.originX !== dragX && this.originY !== dragY) {
        ...
        const height = getTimePickerStandardAbsolutePosition(
            absolutePosition,
    ...
    return (
        <div className="picker_handler">
          <div
              ...
              style={getTimePickerInitialPointerStyle(height, top, pointerRotate)}
          >
            <div
                ref={r => (this.dragCenter = r)}
                className={`pointer_drag ${draggable ? 'draggable' : ''}`}
                style={getTimePickerRotateStyle(-pointerRotate)}
